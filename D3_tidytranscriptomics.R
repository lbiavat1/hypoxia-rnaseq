...
saveDir <- file.path(dirPath, "results_D3_TissueCondition")
savePath <- saveDir
dir.create(saveDir)
...
dds <- DESeqDataSetFromMatrix(countData = rowname_cts, colData = col.data, 
                              design = ~ ID + Tissue + Condition)
dds$group <- factor(paste0(dds$Tissue, "_", dds$Condition),
                    levels = c("BM_Norm", "PB_Norm", "BM_Hypo", "PB_Hypo"))
...
counts_scal_PCA %>%
  ...
  theme_bw()
ggsave(file.path(plotDir, "PCA_top100.pdf"), device = "jpeg")
...
tt_mds %>%
  ...
  geom_text_repel(aes(label = ""), show.legend = FALSE) +
  theme_bw()
ggsave(file.path(plotDir, "MDS_top100.pdf"), device = "jpeg")
...
hm <- counts_scaled %>%
  ...
  # extract most variable genes
  keep_variable( .abundance = counts_scaled, top = 1000) %>%
...
strip_chart <-
  ...
  # make stripchart
  ggplot(aes(x = group, y = counts_scaled + 1, fill = group, label = "")) +
  geom_boxplot() +
  geom_jitter() +
  facet_wrap(~ feature) +
  scale_y_log10()+
...
pdf(file = file.path(plotDir, "stripchart_D3.pdf"))
strip_chart
dev.off()
